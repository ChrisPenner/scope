...
class mainWindowHandler(object):
    ...
    def initMainWindow(self):
        ...
        self.ui.listDB.setSelectionMode(
            QtGui.QAbstractItemView.MultiSelection)
        self.ui.listTags.setSelectionMode(
            QtGui.QAbstractItemView.MultiSelection)
        ...
        self.liDB.qtList.itemSelectionChanged.connect(
            self.liDB.selectionChanged)
        self.liDB.qtList.itemSelectionChanged.connect(
            self.setTags)
        ...
        # self.liTags.qtList.itemSelectionChanged.connect(
            # self.liTags.selectionChanged)
    ...
    def setTags(self):
        ...
        self.liTags.clear()
        databases = sorted(
            [x.text() for x in self.liDB.qtList.selectedItems()])
        for db in databases:
            self.liTags.addItems(self.databases[db].tags)
            self.liTags.setSelection()

    def displayEntries(self):
        entries = self.profile.getMatchingEntries(
            self.liDB.getSelected(), self.liTags.getSelected())
